...
- Secret-scan detections fail CI by default; any exception requires explicit security approval via **both** PR labels: `security-review-approved` and `secret-scan-override-approved`.
- On protected branches, require the status check `Secret scan (required)` before merge (see `.github/branch-protection.required-checks.md`).
