#!/usr/bin/env bash
# Shared secret scanning policy for local checks and CI.

FORBIDDEN_FILE_EXTENSIONS_REGEX='\.(keystore|jks|p12|pfx|pem|key|crt|cer|der|mobileprovision|provisionprofile)$'
ALLOWED_EXCLUSION_PATHS_REGEX='^(Gemfile\.lock|yarn\.lock|ios/Podfile\.lock|android/gradle/wrapper/gradle-wrapper\.jar)$'

PATTERN_LABELS=(
  'AWS access key IDs'
  'AWS secret access keys'
  'GCP API keys'
  'Google OAuth client secrets'
  'Firebase tokens'
  'GitHub tokens'
  'Azure storage/account keys'
  'Slack tokens'
  'Generic API key assignments'
  'Generic token assignments'
  'Generic key/private-key assignments'
  'Bearer tokens'
  'Database URLs with embedded credentials'
  'Connection string assignments'
  'Private key headers'
)

PATTERN_REGEXES=(
  '\b(A3T|AKIA|ASIA|AGPA|AIDA|AROA|ANPA)[0-9A-Z]{16}\b'
  '\b(?i)aws(.{0,20})?(secret|access).{0,20}[=:][[:space:]]*[A-Za-z0-9\/+]{40}\b'
  '\bAIza[0-9A-Za-z\-_]{35}\b'
  '\bGOCSPX-[A-Za-z0-9_-]{20,}\b'
  '\bAAAA[A-Za-z0-9_-]{7}:[A-Za-z0-9_-]{140}\b'
  '\b(ghp|gho|ghu|ghs|ghr)_[A-Za-z0-9]{36,255}\b|\bgithub_pat_[A-Za-z0-9_]{80,255}\b'
  '\b(?:AccountKey|SharedAccessKey|DefaultEndpointsProtocol=https;AccountName=)[^\r\n]{16,}\b'
  '\bxox[baprs]-[A-Za-z0-9-]{10,120}\b'
  '\b(api[_-]?key|secret)[[:space:]]*[=:][[:space:]]*["'"'"'`]?[A-Za-z0-9_\-]{16,}["'"'"'`]?\b'
  '\b(access[_-]?token|id[_-]?token|refresh[_-]?token|auth[_-]?token|token)[[:space:]]*[=:][[:space:]]*["'"'"'`]?[A-Za-z0-9._~+\/-]{16,}["'"'"'`]?\b'
  '\b(private[_-]?key|secret[_-]?key|signing[_-]?key)[[:space:]]*[=:][[:space:]]*["'"'"'`]?[A-Za-z0-9._~+\/-]{16,}["'"'"'`]?\b'
  '\bBearer[[:space:]]+[A-Za-z0-9._~+\/-]{20,}=*\b'
  '\b(postgres|postgresql|mysql|mariadb|mongodb|mongodb\+srv|redis|mssql):\/\/[^[:space:]:/]+:[^[:space:]@/]+@[^[:space:]]+'
  '\b(connection[_-]?string|conn(?:ection)?[_-]?str(?:ing)?|database[_-]?url|db[_-]?url)[[:space:]]*[=:][[:space:]]*["'"'"'`]?[A-Za-z0-9;=._~+\/-]{16,}["'"'"'`]?\b'
  '-----BEGIN (RSA |EC |DSA |OPENSSH |ENCRYPTED )?PRIVATE KEY-----'
)
